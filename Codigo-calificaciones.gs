...
function doPost(e) {
  try {
    ...
    switch (data.accion) {
      ...
      case "consultar_entrega_instrumentacion": resultado = consultarEntregaInstrumentacion_(data); break;
      case "configurar_entrega_instrumentacion": resultado = configurarEntregaInstrumentacion_(data); break;
      case "registrar_entrega_instrumentacion": resultado = registrarEntregaInstrumentacion_(data); break;
      case "listar_entregas_instrumentacion": resultado = listarEntregasInstrumentacion_(data); break;
      case "resolver_entrega_instrumentacion": resultado = resolverEntregaInstrumentacion_(data); break;
      case "consultar_entrega_cad": resultado = consultarEntregaCAD_(data); break;
      case "configurar_entrega_cad": resultado = configurarEntregaCAD_(data); break;
...
/* ENTREGAS POR CAPTURA · INSTRUMENTACIÓN */
function claveEntregaInstrumentacion_(){return "ENTREGA_CONFIG|INSTRUMENTACION";}
function consultarEntregaInstrumentacion_(){const texto=PropertiesService.getScriptProperties().getProperty(claveEntregaInstrumentacion_());const config=texto?JSON.parse(texto):{abierta:false,actividad:"ACTIVIDAD_1",titulo:"Actividad 1"};return{ok:true,abierta:Boolean(config.abierta),actividad:config.actividad,titulo:config.titulo||"Actividad de Instrumentación"};}
function configurarEntregaInstrumentacion_(data){validarDocente_(data);const actividad=String(data.actividad||"ACTIVIDAD_1").trim().toUpperCase().replace(/[^A-Z0-9_-]/g,"_");const titulo=String(data.titulo||"Actividad de Instrumentación").trim().slice(0,120);const config={abierta:Boolean(data.abierta),actividad:actividad,titulo:titulo,actualizada:new Date().toISOString()};PropertiesService.getScriptProperties().setProperty(claveEntregaInstrumentacion_(),JSON.stringify(config));return{ok:true,config:config,mensaje:config.abierta?"✅ Entrega habilitada.":"🔒 Entrega cerrada."};}
function hojaEntregasInstrumentacion_(){const libro=SpreadsheetApp.openById(ID_HOJA_CALIFICACIONES);let hoja=libro.getSheetByName("ENTREGAS_INSTRUMENTACION");if(!hoja){hoja=libro.insertSheet("ENTREGAS_INSTRUMENTACION");hoja.appendRow(["ID","FECHA","NOMBRE","MATRICULA","ACTIVIDAD","ARCHIVO","HASH_EXACTO","HASH_VISUAL","SIMILITUD","COINCIDE_CON","ESTADO","CALIFICACION","OBSERVACION"]);hoja.setFrozenRows(1);}return hoja;}
function carpetaEntregasInstrumentacion_(){const propiedades=PropertiesService.getScriptProperties(),id=propiedades.getProperty("CARPETA_ENTREGAS_INSTRUMENTACION");if(id){try{return DriveApp.getFolderById(id);}catch(e){}}const carpeta=DriveApp.createFolder("IM3CB_ENTREGAS_INSTRUMENTACION");propiedades.setProperty("CARPETA_ENTREGAS_INSTRUMENTACION",carpeta.getId());return carpeta;}
function registrarEntregaInstrumentacion_(data){return conBloqueo_(function(){const config=consultarEntregaInstrumentacion_();if(!config.abierta)return{ok:false,mensaje:"La entrega está cerrada por el docente."};const alumno=obtenerAlumno_(Object.assign({},data,{curso:"INSTRUMENTACION",evaluacion:config.actividad})),imagen=String(data.imagenBase64||""),coincidencia=imagen.match(/^data:image\/(png|jpeg|jpg|webp);base64,(.+)$/);if(!coincidencia)return{ok:false,mensaje:"La captura no tiene un formato válido."};if(coincidencia[2].length>2200000)return{ok:false,mensaje:"La captura es demasiado grande. Vuelve a seleccionarla para comprimirla."};const hoja=hojaEntregasInstrumentacion_(),filas=hoja.getLastRow()>1?hoja.getRange(2,1,hoja.getLastRow()-1,13).getValues():[];if(filas.some(function(f){return String(f[3]).trim()===alumno.matricula&&String(f[4])===config.actividad;}))return{ok:false,mensaje:"Ya existe una entrega de esta matrícula para la actividad."};const hashExacto=String(data.hashExacto||"").toLowerCase(),hashVisual=String(data.hashVisual||"").toLowerCase();let mejor=null;filas.forEach(function(f){if(String(f[4])!==config.actividad)return;const exacta=hashExacto&&hashExacto===String(f[6]).toLowerCase(),distancia=distanciaHammingHex_(hashVisual,f[7]),porcentaje=exacta?100:(hashVisual&&distancia!==9999?Math.max(0,Math.round((1-distancia/(hashVisual.length*4))*100)):0);if(!mejor||porcentaje>mejor.porcentaje)mejor={porcentaje:porcentaje,id:String(f[0]),nombre:String(f[2]),archivo:String(f[5])};});const sospechosa=Boolean(mejor&&mejor.porcentaje>=88),extension=coincidencia[1]==="jpeg"?"jpg":coincidencia[1],idEntrega=Utilities.getUuid(),nombreSeguro=(alumno.matricula||alumno.nombre).replace(/[^a-zA-Z0-9_-]/g,"_"),blob=Utilities.newBlob(Utilities.base64Decode(coincidencia[2]),"image/"+coincidencia[1],config.actividad+"_"+nombreSeguro+"."+extension),archivo=carpetaEntregasInstrumentacion_().createFile(blob),estado=sospechosa?"REVISION_SIMILITUD":"CALIFICADA",calificacion=sospechosa?"":1,observacion=sospechosa?"Posible similitud visual; requiere confirmación docente.":"Sin coincidencia visual relevante.";hoja.appendRow([idEntrega,new Date(),alumno.nombre,alumno.matricula,config.actividad,archivo.getUrl(),hashExacto,hashVisual,mejor?mejor.porcentaje:0,mejor?mejor.id:"",estado,calificacion,observacion]);if(!sospechosa)guardarCalificacion_({nombre:alumno.nombre,matricula:alumno.matricula,curso:"INSTRUMENTACION",evaluacion:config.actividad,numero:1},1);return{ok:true,sospechosa:sospechosa,similitud:mejor?mejor.porcentaje:0,calificacionProvisional:sospechosa?0:1,mensaje:sospechosa?"Entrega recibida y enviada a revisión docente por similitud visual.":"Entrega recibida. Calificación registrada: 1."};});}
function listarEntregasInstrumentacion_(data){validarDocente_(data);const hoja=hojaEntregasInstrumentacion_();if(hoja.getLastRow()<2)return{ok:true,entregas:[]};const filas=hoja.getRange(2,1,hoja.getLastRow()-1,13).getValues(),mapa={};filas.forEach(function(f){mapa[String(f[0])]=f;});return{ok:true,entregas:filas.slice().reverse().map(function(f){const comparada=mapa[String(f[9])]||[];return{id:String(f[0]),fechaTexto:Utilities.formatDate(new Date(f[1]),ZONA_HORARIA,"dd/MM/yyyy HH:mm"),nombre:String(f[2]),matricula:String(f[3]),actividad:String(f[4]),archivo:String(f[5]),similitud:Number(f[8])||0,coincideCon:String(comparada[2]||""),archivoCoincidente:String(comparada[5]||""),estado:String(f[10]),calificacion:String(f[11]),observacion:String(f[12])};})};}
function resolverEntregaInstrumentacion_(data){validarDocente_(data);return conBloqueo_(function(){const hoja=hojaEntregasInstrumentacion_();if(hoja.getLastRow()<2)return{ok:false,mensaje:"Entrega no encontrada."};const ids=hoja.getRange(2,1,hoja.getLastRow()-1,1).getValues();let fila=0;for(let i=0;i<ids.length;i++)if(String(ids[i][0])===String(data.id)){fila=i+2;break;}if(!fila)return{ok:false,mensaje:"Entrega no encontrada."};const datos=hoja.getRange(fila,1,1,13).getValues()[0];if(String(datos[10])!=="REVISION_SIMILITUD")return{ok:false,mensaje:"Esta entrega ya fue resuelta."};const esCopia=String(data.decision)==="confirmar_copia",calificacion=esCopia?0:1;hoja.getRange(fila,11,1,3).setValues([[esCopia?"COPIA_CONFIRMADA":"ORIGINAL_CONFIRMADA",calificacion,esCopia?"El docente confirmó la coincidencia.":"El docente descartó la coincidencia automática."]]);guardarCalificacion_({nombre:String(datos[2]),matricula:String(datos[3]),curso:"INSTRUMENTACION",evaluacion:String(datos[4]),numero:1},calificacion);return{ok:true,mensaje:esCopia?"Calificación 0 registrada tras confirmación docente.":"Calificación 1 registrada; coincidencia descartada."};});}

/* ENTREGAS POR CAPTURA · DIBUJO ASISTIDO POR COMPUTADORA */
function claveEntregaCAD_(){return "ENTREGA_CONFIG|CAD";}
function consultarEntregaCAD_(){const texto=PropertiesService.getScriptProperties().getProperty(claveEntregaCAD_());const config=texto?JSON.parse(texto):{abierta:false,actividad:"ACTIVIDAD_1",titulo:"Actividad 1"};return{ok:true,abierta:Boolean(config.abierta),actividad:config.actividad,titulo:config.titulo||"Actividad de CAD"};}
function configurarEntregaCAD_(data){validarDocente_(data);const actividad=String(data.actividad||"ACTIVIDAD_1").trim().toUpperCase().replace(/[^A-Z0-9_-]/g,"_"),titulo=String(data.titulo||"Actividad de CAD").trim().slice(0,120),config={abierta:Boolean(data.abierta),actividad:actividad,titulo:titulo,actualizada:new Date().toISOString()};PropertiesService.getScriptProperties().setProperty(claveEntregaCAD_(),JSON.stringify(config));return{ok:true,config:config,mensaje:config.abierta?"✅ Entrega de CAD habilitada.":"🔒 Entrega de CAD cerrada."};}
function hojaEntregasCAD_(){const libro=SpreadsheetApp.openById(ID_HOJA_CALIFICACIONES);let hoja=libro.getSheetByName("ENTREGAS_CAD");if(!hoja){hoja=libro.insertSheet("ENTREGAS_CAD");hoja.appendRow(["ID","FECHA","NOMBRE","MATRICULA","ACTIVIDAD","ARCHIVO","HASH_EXACTO","HASH_VISUAL","SIMILITUD","COINCIDE_CON","ESTADO","CALIFICACION","OBSERVACION"]);hoja.setFrozenRows(1);}return hoja;}
function carpetaEntregasCAD_(){const propiedades=PropertiesService.getScriptProperties(),id=propiedades.getProperty("CARPETA_ENTREGAS_CAD");if(id){try{return DriveApp.getFolderById(id);}catch(e){}}const carpeta=DriveApp.createFolder("IM3CB_ENTREGAS_CAD");propiedades.setProperty("CARPETA_ENTREGAS_CAD",carpeta.getId());return carpeta;}
function registrarEntregaCAD_(data){return conBloqueo_(function(){const config=consultarEntregaCAD_();if(!config.abierta)return{ok:false,mensaje:"La entrega está cerrada por el docente."};const alumno=obtenerAlumno_(Object.assign({},data,{curso:"CAD",evaluacion:config.actividad})),imagen=String(data.imagenBase64||""),coincidencia=imagen.match(/^data:image\/(png|jpeg|jpg|webp);base64,(.+)$/);if(!coincidencia)return{ok:false,mensaje:"La captura no tiene un formato válido."};if(coincidencia[2].length>2200000)return{ok:false,mensaje:"La captura es demasiado grande. Vuelve a seleccionarla para comprimirla."};const hoja=hojaEntregasCAD_(),filas=hoja.getLastRow()>1?hoja.getRange(2,1,hoja.getLastRow()-1,13).getValues():[];if(filas.some(function(f){return String(f[3]).trim()===alumno.matricula&&String(f[4])===config.actividad;}))return{ok:false,mensaje:"Ya existe una entrega de esta matrícula para la actividad."};const hashExacto=String(data.hashExacto||"").toLowerCase(),hashVisual=String(data.hashVisual||"").toLowerCase();let mejor=null;filas.forEach(function(f){if(String(f[4])!==config.actividad)return;const exacta=hashExacto&&hashExacto===String(f[6]).toLowerCase(),distancia=distanciaHammingHex_(hashVisual,f[7]),porcentaje=exacta?100:(hashVisual&&distancia!==9999?Math.max(0,Math.round((1-distancia/(hashVisual.length*4))*100)):0);if(!mejor||porcentaje>mejor.porcentaje)mejor={porcentaje:porcentaje,id:String(f[0]),nombre:String(f[2]),archivo:String(f[5])};});const sospechosa=Boolean(mejor&&mejor.porcentaje>=88),extension=coincidencia[1]==="jpeg"?"jpg":coincidencia[1],idEntrega=Utilities.getUuid(),nombreSeguro=(alumno.matricula||alumno.nombre).replace(/[^a-zA-Z0-9_-]/g,"_"),blob=Utilities.newBlob(Utilities.base64Decode(coincidencia[2]),"image/"+coincidencia[1],config.actividad+"_"+nombreSeguro+"."+extension),archivo=carpetaEntregasCAD_().createFile(blob),estado=sospechosa?"REVISION_SIMILITUD":"CALIFICADA",calificacion=sospechosa?"":1,observacion=sospechosa?"Posible similitud visual; requiere confirmación docente.":"Sin coincidencia visual relevante.";hoja.appendRow([idEntrega,new Date(),alumno.nombre,alumno.matricula,config.actividad,archivo.getUrl(),hashExacto,hashVisual,mejor?mejor.porcentaje:0,mejor?mejor.id:"",estado,calificacion,observacion]);if(!sospechosa)guardarCalificacion_({nombre:alumno.nombre,matricula:alumno.matricula,curso:"CAD",evaluacion:config.actividad,numero:1},1);return{ok:true,sospechosa:sospechosa,similitud:mejor?mejor.porcentaje:0,calificacionProvisional:sospechosa?0:1,mensaje:sospechosa?"Entrega recibida y enviada a revisión docente por similitud visual.":"Entrega recibida. Calificación registrada: 1."};});}
function listarEntregasCAD_(data){validarDocente_(data);const hoja=hojaEntregasCAD_();if(hoja.getLastRow()<2)return{ok:true,entregas:[]};const filas=hoja.getRange(2,1,hoja.getLastRow()-1,13).getValues(),mapa={};filas.forEach(function(f){mapa[String(f[0])]=f;});return{ok:true,entregas:filas.slice().reverse().map(function(f){const comparada=mapa[String(f[9])]||[];return{id:String(f[0]),fechaTexto:Utilities.formatDate(new Date(f[1]),ZONA_HORARIA,"dd/MM/yyyy HH:mm"),nombre:String(f[2]),matricula:String(f[3]),actividad:String(f[4]),archivo:String(f[5]),similitud:Number(f[8])||0,coincideCon:String(comparada[2]||""),archivoCoincidente:String(comparada[5]||""),estado:String(f[10]),calificacion:String(f[11]),observacion:String(f[12])};})};}
function resolverEntregaCAD_(data){validarDocente_(data);return conBloqueo_(function(){const hoja=hojaEntregasCAD_();if(hoja.getLastRow()<2)return{ok:false,mensaje:"Entrega no encontrada."};const ids=hoja.getRange(2,1,hoja.getLastRow()-1,1).getValues();let fila=0;for(let i=0;i<ids.length;i++)if(String(ids[i][0])===String(data.id)){fila=i+2;break;}if(!fila)return{ok:false,mensaje:"Entrega no encontrada."};const datos=hoja.getRange(fila,1,1,13).getValues()[0];if(String(datos[10])!=="REVISION_SIMILITUD")return{ok:false,mensaje:"Esta entrega ya fue resuelta."};const esCopia=String(data.decision)==="confirmar_copia",calificacion=esCopia?0:1;hoja.getRange(fila,11,1,3).setValues([[esCopia?"COPIA_CONFIRMADA":"ORIGINAL_CONFIRMADA",calificacion,esCopia?"El docente confirmó la coincidencia.":"El docente descartó la coincidencia automática."]]);guardarCalificacion_({nombre:String(datos[2]),matricula:String(datos[3]),curso:"CAD",evaluacion:String(datos[4]),numero:1},calificacion);return{ok:true,mensaje:esCopia?"Calificación 0 registrada tras confirmación docente.":"Calificación 1 registrada; coincidencia descartada."};});}

/* VISIBILIDAD DOCENTE · PROBLEMA DE RED DE TUBERÍAS */
function consultarProblemaTuberias_(){const texto=PropertiesService.getScriptProperties().getProperty("ACTIVIDAD_VISIBLE|PROBLEMA_TUBERIAS_1"),config=texto?JSON.parse(texto):{visible:false};return{ok:true,visible:Boolean(config.visible),actualizada:config.actualizada||""};}
function configurarProblemaTuberias_(data){validarDocente_(data);const config={visible:Boolean(data.visible),actualizada:new Date().toISOString()};PropertiesService.getScriptProperties().setProperty("ACTIVIDAD_VISIBLE|PROBLEMA_TUBERIAS_1",JSON.stringify(config));return{ok:true,visible:config.visible,mensaje:config.visible?"✅ Problema de tuberías publicado para todos los alumnos.":"🔒 Problema de tuberías oculto para los alumnos."};}
